...
async def _create_batch(file_count: int) -> str:
    ...
    Uses BatchService so the audit log entry and cache invalidation are
    handled by the service layer — not written directly to the ORM.
    """
    ...
    try:
        async with AsyncSessionLocal() as session:
            svc   = BatchService(session, cache)
            batch = await svc.create_batch(data=BatchCreate(file_count=file_count))
    finally:
        await cache.close()

    log.info("batch_created id=%s file_count=%d", batch.id, file_count)
    return batch.id
